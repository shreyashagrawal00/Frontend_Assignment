# 📘 GLA University

## **Frontend Assignment File**

### **Student Details**

**Name:** Shreyash Agrawal
**Roll No:** 59 (FD)
**University No:** 2415001544
**Course:** B.Tech-CS (FullStack)

---

# 📄 **Index**

1. Task 1 – Dynamic To-Do List
2. Task 2 – Live Character Counter
3. Task 3 – Image Carousel
4. Task 4 – Form Validation App
5. Task 5 – Interactive Product Filter

---

# ✅ **Task 1: Dynamic To-Do List**

Short Description:

* Add tasks dynamically
* Edit & Delete functionality
* Updates without refresh
* **Bonus:** Saves tasks using localStorage

---

# ✅ **Task 2: Live Character Counter**

Short Description:

* Real-time character update
* Typing stops at max limit
* **Bonus:** Counter color changes

---

# ✅ **Task 3: Image Carousel**

Short Description:

* Slideshow with 5+ images
* Next/Prev buttons
* Auto-slide every 3 seconds
* **Bonus:** Pause on hover

---

# ✅ **Task 4: Form Validation App**

Short Description:

* Validate Name, Email, Password
* Inline errors (no alerts)
* **Bonus:** Red/Green borders

---

# ✅ **Task 5: Interactive Product Filter**

Short Description:

* Filter by category
* Dynamic product listing
* **Bonus:** Search bar filter

---
